...
from abnamro_bsrc_etl.config.exceptions import DialExtractionError
from abnamro_bsrc_etl.config.process import ProcessLogConfig
from abnamro_bsrc_etl.staging.extract_dial_data import ExtractDialData
...
def dial_load(  # noqa: C901
    ...
    triggers, triggers_archive = (
        extraction.get_triggers(data=matching_meta_trigger_data, matching=matching)
    ...
    for file_name_archive, archive_config in triggers_archive.items():
        ...
        if not extraction.move_archive_file(trigger_file_name=file_name_archive):
